...
program
	...
	.action(async (directory: string, options: { template: string; authentication: string | null | undefined }) => {
		console.clear();
		output.log(logo);
		output.log();
		output.log(
		...
		const Ninjaninja140CheckBuffer = output.buffer('Checking Bracketed Repositories for the template supplied...');
		...
		if (!repo) {
			GitCloneBuffer.fail(`${chalk.bold(chalk.red('ERR!'))} There was an error while downloading the Template.`);
			return output.log([
				'If you think that this error was a mistake, try running the command again!',
				'If this error continues to persist, open an issue on the TemplateCLI Repository here: https://github.com/bracketed/templatecli/issues',
			]);
